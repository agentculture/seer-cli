...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope antoine
...
scope:** the default personal scope is **private** (`--scope antoine
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `antoine`). Private records are served only to a recall in the same
